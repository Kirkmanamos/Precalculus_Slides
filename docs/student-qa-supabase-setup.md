# Student Q&A Supabase Setup (5.1 Pilot)

This setup enables anonymous student question submissions from the static GitHub Pages deck and teacher moderation with a passcode.
...
Edit [`5.1-fundamental-identities.html`](/Users/kirkmanamos/Documents/GitHub/precalculus_slides/5.1-fundamental-identities.html) and update `QA_CONFIG`:
1. `SUPABASE_URL`
2. `SUPABASE_ANON_KEY`
3. `EDGE_BASE_URL`
...
Insert a session row (replace placeholders):
...
Teacher then clicks `Unlock Moderation`, enters passcode, and can approve/hide questions.
